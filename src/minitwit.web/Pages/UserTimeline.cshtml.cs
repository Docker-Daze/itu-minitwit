...
public class UserTimelineModel : PageModel
{
    ...
    public async Task<ActionResult> OnGet(string user, [FromQuery] int? page)
    {
        ...
        Messages = await _messageRepository.GetMessagesUserTimeline(user, _currentPage).ConfigureAwait(false);
        return Page();
    }
    ...
    public async Task<bool> IsFollowing(string who)
    {
        ...
        {
            return await _userRepository.IsFollowing(User.Identity.Name, who).ConfigureAwait(false);
        }
        else return false;
    }
    ...
    public async Task<IActionResult> OnPostFollowAsync(string user)
    {
        ...
        if (await _userRepository.IsFollowing(User.Identity.Name!, user).ConfigureAwait(false))
        {
