...
public interface IMessageRepository
{
    ...
    public Task<List<MessageDTO>> GetMessagesSpecifiedAmount(int amount);
    public Task<List<MessageDTO>> GetMessagesUserTimeline(string username, int page);
    public Task<List<MessageDTO>> GetMessagesFromUsernameSpecifiedAmount(string username, int amount);
